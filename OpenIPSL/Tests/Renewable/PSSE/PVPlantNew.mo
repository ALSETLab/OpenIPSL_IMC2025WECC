...
model PVPlantNew
  ...
  extends BaseClasses.SMIBRenewable(SysData(fn=60), freq(k=SysData.fn),
    pwFault(t2=3.00)                                                  );
  Electrical.Renewables.PSSE.PVNew
                                pVNew(
    P_0=1500000,
    Q_0=-5665800,
    v_0=1,
    ...
    redeclare OpenIPSL.Electrical.Renewables.PSSE.InverterInterface.REGCA1
      RenewableGenerator,
    redeclare OpenIPSL.Electrical.Renewables.PSSE.ElectricalController.REECDU1
      RenewableController(vref0=0,
        pfflag=true,
      vflag=true,
      qflag=true,
      pqflag=true,
...
equation
  connect(pVNew.pwPin, GEN1.p)
    annotation (Line(points={{-40,0},{-30,0}}, color={0,0,255}));
  connect(freq.y, pVNew.FREQ) annotation (Line(points={{-75,0},{-68,0},{-68,-4},
          {-62,-4}}, color={0,0,127}));
  connect(pwVoltage.vi, pVNew.regulate_vi)
    annotation (Line(points={{-36.6,30},{-44,30},{-44,10}}, color={0,0,127}));
  connect(pwVoltage.vr, pVNew.regulate_vr) annotation (Line(points={{-36.6,33.6},
          {-56,33.6},{-56,10}}, color={0,0,127}));
  connect(pVNew.branch_ir, pwCurrent.ir) annotation (Line(points={{-44,-10},{-44,
          -16},{-17.6,-16},{-17.6,-6.6}}, color={0,0,127}));
  connect(pwCurrent.ii, pVNew.branch_ii) annotation (Line(points={{-14,-6.6},{-14,
